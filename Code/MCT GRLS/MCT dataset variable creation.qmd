...
- A dog's main lifestyle is its uniquely most frequently recorded raw role. The
  previous `slice_max(..., with_ties = FALSE)` silently selected one role on a
  tie. The shared helper now stops instead; the current source snapshot contains
  403 tied dogs and therefore requires an explicit subject-matter rule before an
  analysis dataset can be accepted.
