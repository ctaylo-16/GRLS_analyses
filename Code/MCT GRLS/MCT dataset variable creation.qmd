...
```{r}
palladia <- c("palladia","toceranib", "toceranib phosphate")
```

```{r}
medications_5y  <- medications_5y  %>%
  mutate(palladia_use = if_else(
    str_detect(tolower(medication_ingredients), str_c(palladia, collapse = "|")),
    "palladia_yes",
    "palladia_no"
  ))

#a count of these for each dog exc if palladia no then just palladia no
palladia_5y_meds_per_dog <- medications_5y  %>%
  group_by(subject_id, palladia_use) %>%
  summarise(count = n(), .groups = "drop")
  #remove rows where if subject has ANY palladia_yes value then remove the palladia_no row/s for that subject id
  # Step 1: Get subject_ids that have palladia_yes
palladia_yes_subjects <- palladia_5y_meds_per_dog %>%
  filter(palladia_use == "palladia_yes") %>%
  pull(subject_id)

# Step 2: Keep palladia_yes rows or palladia_no only if no palladia_yes exists for that dog
palladia_5y_meds_per_dog_clean <- palladia_5y_meds_per_dog %>%
  filter(palladia_use == "palladia_yes" | !(subject_id %in% palladia_yes_subjects))

#calculate palladia yes quartiles
palladia_yes <- palladia_5y_meds_per_dog_clean %>%
  filter(palladia_use=="palladia_yes")

palladia_summary <- palladia_yes %>%
  summarise(
    min=min(count),
    Q1 = quantile(count, 0.25, na.rm = TRUE),
    median_count_per_dog = quantile(count, 0.50, na.rm = TRUE),  # Use quantile for clarity
...
#NB is not actually quartiles split
#NB also the count column is counting rows for each dog, so the no_palladia_admins will also have a count for each medical row they DO NOT HAVE palladias for 
palladia_5y_meds_per_dog_clean_quartiles <- palladia_5y_meds_per_dog_clean %>%
  mutate(palladia_amount = case_when(
    palladia_use == "palladia_no" ~ "no_palladia_admin",
    palladia_use == "palladia_yes" ~ as.character(count)
  ))
#then filt down to unique single row for each dog of its count or palladia no, then join back to the 3044 dogs and if no count or no palladia no then "no_palladia_recorded_in_time_period"
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib%>% left_join(palladia_5y_meds_per_dog_clean_quartiles, by="subject_id")

#now replace NAs in steroid_quantile column with "no_med_records_this_time_period"
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia %>%
    mutate(palladia_amount = replace_na(palladia_amount, "no_med_use_recorded"))
...
#rename steroid_quantile to make clear its a 5y column
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia %>%
  rename(palladia_5y_amount = palladia_amount)

###redo indices asnew column

dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia <-
  dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia %>%
  mutate(across(263:264, ~ case_when(
    is.na(.) ~ "no_medication_records_for_this_time_period",
    TRUE ~ as.character(.)
  )))
```
...
```{r}
masivet <-c("masitinib","masivet")
```

```{r}
medications_5y  <- medications_5y  %>%
  mutate(masivet_use = if_else(
    str_detect(tolower(medication_ingredients), str_c(masivet, collapse = "|")),
    "masivet_yes",
    "masivet_no"
  ))

#a count of these for each dog exc if masivet no then just masivet no
masivet_5y_meds_per_dog <- medications_5y  %>%
  group_by(subject_id, masivet_use) %>%
  summarise(count = n(), .groups = "drop")
  #remove rows where if subject has ANY masivet_yes value then remove the masivet_no row/s for that subject id
  # Step 1: Get subject_ids that have masivet_yes
masivet_yes_subjects <- masivet_5y_meds_per_dog %>%
  filter(masivet_use == "masivet_yes") %>%
  pull(subject_id)

# Step 2: Keep masivet_yes rows or masivet_no only if no masivet_yes exists for that dog
masivet_5y_meds_per_dog_clean <- masivet_5y_meds_per_dog %>%
  filter(masivet_use == "masivet_yes" | !(subject_id %in% masivet_yes_subjects))

#calculate masivet yes quartiles
masivet_yes <- masivet_5y_meds_per_dog_clean %>%
  filter(masivet_use=="masivet_yes")

masivet_summary <- masivet_yes %>%
  summarise(
    min=min(count),
    Q1 = quantile(count, 0.25, na.rm = TRUE),
    median_count_per_dog = quantile(count, 0.50, na.rm = TRUE),  # Use quantile for clarity
...
#NB is not actually quartiles split
#NB also the count column is counting rows for each dog, so the no_masivet_admins will also have a count for each medical row they DO NOT HAVE masivets for 
masivet_5y_meds_per_dog_clean_quartiles <- masivet_5y_meds_per_dog_clean %>%
  mutate(masivet_amount = case_when(
    masivet_use == "masivet_no" ~ "no_masivet_admin",
    masivet_use == "masivet_yes" ~ as.character(count)
  ))
#then filt down to unique single row for each dog of its count or masivet no, then join back to the 3044 dogs and if no count or no masivet no then "no_masivet_recorded_in_time_period"
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia%>% left_join(masivet_5y_meds_per_dog_clean_quartiles, by="subject_id")

#now replace NAs in steroid_quantile column with "no_med_records_this_time_period"
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet %>%
    mutate(masivet_amount = replace_na(masivet_amount, "no_med_use_recorded"))
...
#rename steroid_quantile to make clear its a 5y column
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet  %>%
  rename(masivet_5y_amount = masivet_amount)

###redo indices asnew column

dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet  <-
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet  %>%
  mutate(across(265:266, ~ case_when(
    is.na(.) ~ "no_medication_records_for_this_time_period",
    TRUE ~ as.character(.)
  )))
```
...
```{r}
write.csv(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet,"C:/Users/ctaylor18/GitHub/GRLS_analyses/Output/GRLS_MCT_variables_dataset_interim.csv")
```
...
## add back onto df
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet_neutering <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet %>% left_join(profile_abr, by="subject_id")
```
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet2 <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet %>% left_join(neut_data_single_row,by="subject_id")
#delete dupe columns
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3 <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet2 %>%
mutate(
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight <-  dog_weight%>%
  left_join(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3,by="subject_id")
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight2 <- 
  dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight %>%
...
```{r}
#avg weight to kg
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight3<- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight2 %>%
...
#avg height to cm
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4<- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight3 %>%
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_abr <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4 %>%
  dplyr::select(subject_id,295:297)

#rejoin back
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_unique<- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3 %>% left_join(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_abr,by="subject_id")
```

```{r}
write.csv(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_unique,"C:/Users/ctaylor18/GitHub/GRLS_analyses/Output/GRLS_MCT_variables_dataset_interim.csv")
```
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_unique %>%
...
cohort_abr <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity %>%
  dplyr::select(c(subject_id,end_date2,had_MCT,year_in_study_diagnosis_or_final_record_year))
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_all_rows <- activity_all_y_all_rows %>%
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_all_rows_2<- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_all_rows[!duplicated(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_all_rows), ]

#make sure year is in numeric
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_all_rows_2$year_in_study_diagnosis_or_final_record_year  <- as.numeric(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_all_rows_2$year_in_study_diagnosis_or_final_record_year )

cohort_5y_prior <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_all_rows_2 %>%
  group_by(subject_id) %>%
  filter(year_in_study >= (year_in_study_diagnosis_or_final_record_year - 5)) %>%
  summarise(
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity %>% 
  left_join(cohort_5y_prior, by="subject_id")

#for any dogs with NAs recode "reocrds_not_available"
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity %>%
  mutate(across(286:291, ~case_when(
      is.na(.) ~ "no_records_for_this_time_period",
    TRUE ~ as.character(.)  
  )))
...
```{r}
colnames(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity)

#remove unnecessary columns:
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity2 <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity %>%
  dplyr::select(-c(record_date.x,X))

sum(is.na(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity2))

#check which columns have most NAs and decide how best to replace
na_summary <- colSums(is.na(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity2))
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity3 <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity2 %>%
  mutate(across(2:21, ~ replace(., is.na(.), "records_not_available")))
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity3 <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity3 %>%
  mutate(across(36:262, ~ replace(., is.na(.), "records_not_available")))
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity3 <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity3 %>%
  mutate(across(276:300, ~ replace(., is.na(.), "records_not_available")))

#check NAs now
na_summary2 <- colSums(is.na(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity3))
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity3 <- 
  dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity3 %>%
  mutate(avg_weight_5y_prior_endpoint = replace(avg_weight_5y_prior_endpoint_kg, 
                                                 is.na(avg_weight_5y_prior_endpoint_kg), 
                                                 "records_not_available"))


dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity_tidied <- 
  dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity3 %>%
...
```{r}
write.csv(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_systemic_pred_lokivetmab_oclacitinib_palladia_masivet3_weight4_activity_endpoint_activity_tidied,"C:/Users/ctaylor18/GitHub/GRLS_analyses/Output/GRLS_MCT_variables_dataset.csv")
```
