...
#remove 3 rows from the HSA_dogs_2 that are dogs in the multi HSA list and have 2 seperate records with same date
#dog with 2 cardiac records on same date -094-014074

HSA_dogs2 <- HSA_dogs2 %>%
  group_by(subject_id) %>%
  filter(!(subject_id == "094-014074") | row_number() == 1) %>%
  ungroup()
#second dog with same issue - 	094-030098
HSA_dogs2 <-HSA_dogs2 %>%
    group_by(subject_id) %>%
  filter(!(subject_id == "094-030098") | row_number() == 1) %>%
  ungroup()

#3rd dog with seperate definitive splenci and hepatic on same day,just keep one row - 094-004453
HSA_dogs2 <-HSA_dogs2 %>%
    group_by(subject_id) %>%
  filter(!(subject_id == "094-004453") | row_number() == 1) %>%
  ungroup()
```
...
Count of how many dogs had \>1 tumour location recorded

```{r}
multi_locations <- HSA_dogs2 %>%
...
### Log rank to compare if visceral VS cutaneous MST sig different

```{r}

HSA_dogs4 <- HSA_dogs4 %>%
  mutate(visceral_or_cutaneous = if_else(grepl("cut", tracked_condition, ignore.case = TRUE))



survdiff(Surv(time = time_between_diagnosis_date_death_date_days, event = status) ~ visceral_or_cutaneous, data = HSA_dogs4)

         
           
```

Log rankf or all HSA locations

```{r}
surv_object <- Surv(time = HSA_dogs4$time_between_diagnosis_date_death_date_days, event = HSA_dogs4$status)

logrank_test <- survdiff(surv_object ~ cancer_group, data = HSA_dogs4)
logrank_test
...
library(survminer)
pairwise_survdiff(Surv(time_between_diagnosis_date_death_date_days, status) ~ cancer_group, data = HSA_dogs4)
