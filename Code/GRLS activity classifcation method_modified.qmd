...
```{r}
dog_enrollment<- read.csv(here::here("Data/data version Feb 2024/dog_details.csv"))
```
...
#frequency recoding
never <- c("Never_","Never_Less than once/month","never") 
less_than_weekly <- c("Less than once/month_","Less than once/month_Less than once/month", "Less than once/month_Less than once/week","Less than once/week_Less than once/month","Never_Less than once/week","Less than once/week_",
                      "Less than once/week_Less than once/week","less_than_weekly", "Monthly","Rarely")
min_weekly<-c("Once or twice/week_Once or twice/week","Once or twice/week_","Never_Once or twice/week","Once or twice/week_Less than once/month",
              "Once or twice/week_Less than once/week","Less than once/week_Once or twice/week","Less than once/month_Once or twice/week","min_weekly","Weekly")
...
daily<-c("Once/day_","Less than once/week_Once/day","Never_Once/day", "Once or twice/week_Once/day","Once/day_Once or twice/week","Once/day_Less than once/week",
         ...
         "Once or twice/week_Once/day","Less than once/month_Once/day","daily","Daily"
         )
multi_daily <-c("Once/day_Once/day","Once/day_More than once a day","More than once a day_","Once or twice/week_More than once a day","Less than once/month_More than once a day","More than once a day_Once/day","Never_More than once a day","More than once a day_Less than once/month","More than once a day_Once or twice/week","Less than once/week_More than once a day","More than once a day_More than once a day",
                "More than once a day_Less than once/week","multi_daily","More than daily")
...
totals_avg <- activity_sy1_2 %>%
  ...
  summarise(
    ...
    avg_activity_freq = (mean(sum(total_activity_freq_regroup_num, na.rm = TRUE)) /
                              (n() * 100)) * 100,
    mode_activity_duration_early =
      get_mode(total_activity_duration_regroup),

    mode_activity_intensity_early =
      get_mode(total_activity_intensity_regroup),

    mode_activity_frequency_early =
      get_mode(total_activity_freq_regroup),
...
Study doesnt record duration of exercise for SY3 beyond??

### Data manipulation

#### Total activity pace (will become intensity later for modelling) and frequency over SY3-\>end
...
pace %in% c("Average walk","Slow walk") ~ "low_int",  pace %in% c("Brisk walk","Jog") ~ "mod_int", pace %in% c("Run") ~ "high_int", 
...
Ensure frequency values match the S1-2 values

```{r}
activity_sy3_beyond <- activity_sy3_beyond %>%
  mutate(
    frequency = case_when(
      frequency %in% never ~ "never",
      frequency %in% less_than_weekly ~ "less_than_weekly",
      frequency %in% min_weekly ~ "min_weekly",
      frequency %in% daily ~ "daily",
      frequency %in% multi_daily ~ "multi_daily",
      TRUE ~ "no_freq_recorded" # For values not in any group
    )
  )
...
totals_y3 <- activity_sy3_beyond %>%
  ...
  summarise(
   # avg_activity_duration = (mean(duration_num, na.rm = TRUE)/
    ...
    mode_activity_intensity_rest = get_mode(pace),
    mode_activity_freq_rest=get_mode(frequency),
    mode_activity_level_rest=get_mode(activity_level)
  )
...
totals <- activity_overview %>%
  group_by(subject_id) %>%
  summarise(
    mean_activity_level = (mean(activity_level_num, na.rm = TRUE) / (n() * 100)) * 100,
    mode_activity_level = get_mode(activity_level),
    .groups = "drop"
  )
...
early_life_activity_overview <- activity_overview %>%
  ...
  group_by(subject_id) %>%
  summarise(
    early_activity_mode = get_mode(activity_level),
...
rest_life_activity_overview <- activity_overview %>%
  ...
  group_by(subject_id) %>%
  summarise(
    rest_activity_mode = get_mode(activity_level),
...
whole_life_activity_overview <- activity_overview %>%
  group_by(subject_id) %>%
  summarise(
    whole_activity_mode = get_mode(activity_level),
...
Whilst have got the whole life acitivity intensity from activity overview dataset also want to calculate frequency by combining SY1_2 and SY3 beyond dfs as have made frequency a consistent variable across them:

```{r}
#keep SY0-2 and SY3 beyond same cols only
activity_sy1_2_same <- activity_sy1_2 %>%
  dplyr::select(c(subject_id,year_in_study,total_activity_intensity_regroup_num,total_activity_intensity_regroup,total_activity_freq_regroup,total_activity_freq_regroup_num))
#rename cols to match SY3
activity_sy1_2_same <- activity_sy1_2_same %>%
  rename(pace_num = total_activity_intensity_regroup_num, pace=total_activity_intensity_regroup, frequency_num = total_activity_freq_regroup_num,frequency = total_activity_freq_regroup)
activity_sy3_same<- activity_sy3_beyond %>%
    dplyr::select(c(subject_id,year_in_study,pace,pace_num,frequency,frequency_num))



early_rest <- bind_rows(activity_sy1_2_same,activity_sy3_same)


early_rest_modes <- early_rest %>%
  group_by(subject_id) %>%
  summarise(
    whole_mode_pace = get_mode(pace),
    whole_mode_frequency = get_mode(frequency)
  )


#now add onto that whole life activity overview level df
whole_life_activity_overview <- whole_life_activity_overview %>%
  left_join(early_rest_modes, by="subject_id")
```
...
#incorporate in the mode values generated for SY1_2 for activity duration, intensity and frequency and SY3+ for freq and intensity and level
mode_values_sy1_2 <- activity_sy1_2 %>%
  dplyr::select(c(subject_id,mode_activity_duration_early,mode_activity_intensity_early,mode_activity_frequency_early))%>%
  distinct(subject_id, .keep_all = TRUE)
mode_values_sy3_beyond <- activity_sy3_beyond %>%
  dplyr::select(c(subject_id,mode_activity_intensity_rest,mode_activity_freq_rest,mode_activity_level_rest)) %>%
  distinct(subject_id, .keep_all = TRUE)

#combine the mode values and join into the overview
mode_combined <- mode_values_sy1_2 %>%left_join( mode_values_sy3_beyond,by="subject_id")

#add these onto the early_rest_whole df
early_rest_whole_activity_overview <- early_rest_whole_activity_overview %>%left_join(mode_combined,by="subject_id")

#replace NAs with "records_not_available"
early_rest_whole_activity_overview <- early_rest_whole_activity_overview %>%
  mutate(across(2:24, ~ case_when(
    is.na(.) ~ "no_activity_records_for_this_time_period",
    TRUE ~ as.character(.)
  )))

write.csv(early_rest_whole_activity_overview,here::here("Output/early_rest_whole_activity_level_classification.csv"))
```
...
activity_sy1_2_abr_3 <- activity_sy1_2 %>%
  dplyr::select(c(subject_id,year_in_study, USDA_30min,activity_1h_min,activity_KC_2h_min,avg_activity_duration,avg_activity_intensity,avg_activity_freq,mode_activity_duration_early,mode_activity_frequency_early,mode_activity_intensity_early))


activity_sy3_beyond_abr_3 <- activity_sy3_beyond %>%
  dplyr::select(c(subject_id,year_in_study,
                  avg_activity_freq,avg_activity_intensity,mode_activity_intensity_rest,mode_activity_freq_rest,mode_activity_level_rest))

activity_overview_abr_3 <- activity_overview %>%
  dplyr::select(c(subject_id,year_in_study,mean_activity_level,mode_activity_level))


activity_sy1_2_abr_3 <- activity_sy1_2_abr_3 %>%
  distinct(subject_id, .keep_all = TRUE)  %>%
 # mutate(study_year = "y1_2")%>%
  rename_with(~ paste0(., "_sy1_2"), .cols = 6:8)%>%
...
write.csv(activity_all_3_3, here::here("Output/GRLS_activity_modified_variables_one_row_per_dog_outof100.csv"))
```
...
write.csv(endpoints_activity_all_3_3,here::here("Output/GRLS_activity_modified_variables_endpoints_outof100.csv"))
```
...
```{r}
HSA_cohort <- read.csv(here::here("Output/HSA_study_pop_with_final_date_with_final_study_record_year.csv"))
...
HSA_cohort_activity_all_y_all_rows <- activity_all_y_all_rows %>%
  left_join(HSA_cohort_all,by="subject_id")%>%
  dplyr::select(-c("X"))
...
HSA_cohort_5y_prior <- HSA_cohort_activity_all_y_all_rows %>%
  ...
  summarise(
    ...
    mode_intensity_5y_prior_endpoint=get_mode(pace),
    mode_frequency_5y_prior_endpoint=get_mode(frequency),
    .groups = "drop"
  )
...
prior5y_activity_overview <- HSA_cohort_overview %>%
  ...
  filter(year_in_study >= (year_in_study_diagnosis_or_final_record_year - 5)) %>%
  summarise(
    prior5y_activity_mode = get_mode(activity_level),
...
write.csv(early_rest_whole_5y_activity_overview,here::here("Output/HSA_cohort_early_rest_whole_5y_activity__level_classification.csv"))
```
...
write.csv(HSA_cohort_all_calcs,here::here("Output/HSA_cohort_activity_classification.csv"))
```
