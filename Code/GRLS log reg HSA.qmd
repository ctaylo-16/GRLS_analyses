...
HSA cases vs non-case cohort logistic regression
...
Activity datasets very challenging to explain the cut-offs quartile for interpretation so going to attempt to split into more biologically meaningful categories based on the original `Code/GRLS activity classifcation method_modified.qmd` script. Columns to modify: duration, frequency and intensity in SY1-2; intensity and frequency in SY3 onwards; and intensity and frequency in the five years before the endpoint.
...
NB. decided not to use these in manuscript but will keep here and use for future work on activity
...
-   Counts of cases and non-cases and Chi sq for variables
...
write.csv(univar_table,here::here("Output/HSA_cross_table_case_control_output.csv"))
...
#dependent var needs to be in factor
cohort_abr$had_HSA <- as.factor(cohort_abr$had_HSA)
#need these to be numeric variables
cohort_abr <- cohort_abr %>%
  mutate(across(c(early_activity_mean, rest_activity_mean, whole_activity_mean,prior5y_activity_mean), as.numeric))
...
write.csv(univar_results, here::here("Output/HSA_GRLS_log_reg_univar.csv"), row.names = FALSE)
...
#read in data
univar <- read.csv(here::here("Output/HSA_GRLS_log_reg_univar.csv"))

#function to tidy data and write to csv in a table ready to use for univar
table <- univar_logistic_regression_table(univar,here::here("Output/HSA_GRLS_log_reg_univar_tidied_table.csv"))
...
write.csv(table_2,here::here("Output/HSA_GRLS_log_reg_univar_tidied_table2.csv"))
...
# Save the Word document
print(doc, target = here::here("Output/HSA_GRLS_log_reg_univar_tidied_table2.docx"))
```
...
# If you want to write the output to CSV after running:
write.csv(univar_results_FN, here::here("Output/HSA_GRLS_log_reg_univar_FN_pop.csv"), row.names = FALSE)
...
# If you want to write the output to CSV after running:
write.csv(univar_results_MN, here::here("Output/HSA_GRLS_log_reg_univar_MN_pop.csv"), row.names = FALSE)
#Neut pop
...
# If you want to write the output to CSV after running:
write.csv(univar_results_N, here::here("Output/HSA_GRLS_log_reg_univar_N_pop.csv"), row.names = FALSE)
#ME
explan_neut4 <- c("intact_sired_litters_natural_ever")
univar_results_ME<- univar_logistic_regression(ME_pop, "had_HSA", explan_neut4)

# If you want to write the output to CSV after running:
write.csv(univar_results_ME, here::here("Output/HSA_GRLS_log_reg_univar_ME_pop.csv"), row.names = FALSE)
...
# If you want to write the output to CSV after running:
write.csv(univar_results_F, here::here("Output/HSA_GRLS_log_reg_univar_F_pop.csv"), row.names = FALSE)
...
write.csv(univar_repro,here::here("Output/HSA_GRLS_log_reg_univar_all_repro_pops.csv"), row.names = FALSE)



#format this univar into table for word export
#function to tidy data and write to csv in a table ready to use for univar
table <- univar_logistic_regression_table(univar_repro,here::here("Output/HSA_GRLS_log_reg_univar_all_repro_pops_tidied_table.csv"))
...
write.csv(table_2,here::here("Output/HSA_GRLS_log_reg_univar_all_repro_pops_tidied_table2.csv"))
...
# Save the Word document
print(doc, target = here::here("Output/HSA_GRLS_log_reg_univar_all_repro_pops_tidied_table2.docx"))
```

Run univar log reg to only retain variables where they are sig not just due to NA/not recorded variable - as want to only use these to take forward for multivar analysis

```{r}
#code to check that a variable has at least 2 or more levels for log reg or else will produce errors
...
# Print columns that do not meet the condition so can not use them in select below
...
#recode "no med records" to NA so that univar will run ignoring the missing data here
for (i in cols_to_modify) {
...
#univar_logistic_regression(cohort_no_not_rec, "had_HSA", here::here("Output/HSA_GRLS_log_reg_univar_no_missing.csv"))
...
write.csv(univar_table_no_missing,here::here("Output/HSA_GRLS_log_reg_univar_no_missing.csv"))
...
lrt_pvalues <-read.csv(here::here("Output/HSA_GRLS_log_reg_univar.csv"))
...
pvalues_no_miss <-read.csv(here::here("Output/HSA_GRLS_log_reg_univar_no_missing.csv"))
...
create_corr_matrix(cohort_abr_sig,c(1:114), here::here("Output/hsa_corr_matrix_quartile_logreg.csv"), here::here("Output/hsa_corr_matrix_quartile_logreg2.csv"),here::here("Output/hsa_corr_matrix_quartile_logreg_corrvalue.csv"),here::here("Output/hsa_corr_matrix_quartile_logreg.png"))
```

Correlated variables

```{r}
corr_values <- read.csv(here::here("Output/hsa_corr_matrix_quartile_logreg_corrvalue.csv"))

#ditch rows where variable 1 and 2 values are same as in another row = only have unique corr pairs listed
...
create_corr_matrix(cohort_abr_sig2,c(1:114), here::here("Output/hsa_corr_matrix_quartile_logreg_no_missing.csv"), here::here("Output/hsa_corr_matrix_quartile_logreg2_no_missing.csv"),here::here("Output/hsa_corr_matrix_quartile_logreg_corrvalue_no_missing.csv"),here::here("Output/hsa_corr_matrix_quartile_logreg_no_missing.png"))





corr_values <- read.csv(here::here("Output/hsa_corr_matrix_quartile_logreg_corrvalue_no_missing.csv"))
...
-   Other activity variables - whole,early,rest,prior 5y median/min/max/mode

```{r}
#whole life
model_5g<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + whole_activity_mode , data = cohort_abr_sig, family = binomial)
summary(model_5g)
lrtest(model_4a,model_5g)
logistic.display(model_5g)


model_5h<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + whole_activity_median , data = cohort_abr_sig, family = binomial)
summary(model_5h)
lrtest(model_4a,model_5h)
logistic.display(model_5h)


model_5i<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + whole_activity_min, data = cohort_abr_sig, family = binomial)
summary(model_5i)
lrtest(model_4a,model_5i)
logistic.display(model_5i)


#early life
model_5k<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + early_activity_mode , data = cohort_abr_sig, family = binomial)
summary(model_5g)
lrtest(model_4a,model_5k)
logistic.display(model_5k)


model_5l<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + early_activity_median , data = cohort_abr_sig, family = binomial)
summary(model_5l)
lrtest(model_4a,model_5l)
logistic.display(model_5l)


model_5m<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + early_activity_min, data = cohort_abr_sig, family = binomial)
summary(model_5i)
lrtest(model_4a,model_5m)
logistic.display(model_5m)


#rest of life
model_5n<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + rest_activity_mode , data = cohort_abr_sig, family = binomial)
summary(model_5n)
lrtest(model_4a,model_5n)
logistic.display(model_5n)


model_5o<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + rest_activity_median , data = cohort_abr_sig, family = binomial)
summary(model_5o)
lrtest(model_4a,model_5o)
logistic.display(model_5o)


model_5p<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + rest_activity_min, data = cohort_abr_sig, family = binomial)
summary(model_5p)
lrtest(model_4a,model_5p)
logistic.display(model_5p)

model_5q<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + rest_activity_max  , data = cohort_abr_sig, family = binomial)
summary(model_5q)
lrtest(model_4a,model_5q)
logistic.display(model_5q)
#5y prior
model_5r<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode , data = cohort_abr_sig, family = binomial)
summary(model_5r)
lrtest(model_4a,model_5r)
logistic.display(model_5r)


model_5s<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_median , data = cohort_abr_sig, family = binomial)
summary(model_5s)
lrtest(model_4a,model_5s)
logistic.display(model_5s)


model_5t<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_min, data = cohort_abr_sig, family = binomial)
summary(model_5t)
lrtest(model_4a,model_5t)
logistic.display(model_5t)

model_5u<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_max  , data = cohort_abr_sig, family = binomial)
summary(model_5u)
lrtest(model_4a,model_5u)
logistic.display(model_5u)
```
...
-   all have AIC lower than the splits above so sub in perhaps the prior 5y mode ?
...
model_6b<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + mode_purina_BCS_5y_prior_endpoint, data = cohort_abr_sig, family = binomial)
summary(model_6b)
...
```{r}
model_7<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life, data = cohort_abr_sig, family = binomial)
summary(model_7)
...
```{r}
model_8a<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint, data = cohort_abr_sig, family = binomial)
...
```{r}
model_9<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint+ main_lifestyle_category_mode_5yrs_prior, data = cohort_abr_sig, family = binomial)
summary(model_9)
...
```{r}
model_10a<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint + malig_cancers_YN_no_HSA, data = cohort_abr_sig, family = binomial)
summary(model_10a)
...
model_10b<- glm(had_HSA ~ age_at_final_date_GB  + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint + malig_cancers_YN_no_HSA, data = cohort_abr_sig, family = binomial)
summary(model_10b)
logistic.display(model_10b)

model_10c<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint , data = cohort_abr_sig, family = binomial)
summary(model_10c)
logistic.display(model_10c)


model_10d<- glm(had_HSA ~ age_at_final_date_GB  +malig_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint , data = cohort_abr_sig, family = binomial)
summary(model_10d)
...
```{r}
model_11<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint + fenbendazole_ever, data = cohort_abr_sig, family = binomial)
...
```{r}
model_12<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever, data = cohort_abr_sig, family = binomial)
...
-   heats_before_neutering_tercile
    -   heats themselves are not sig

```{r}
model_13<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever + heats_before_neutering_terciles, data = cohort_abr_sig, family = binomial)
...
```{r}
model_14<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever +  heating_fuel_primary_mode_5yrs_prior, data = cohort_abr_sig, family = binomial)
...
```{r}
model_15<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever + any_treated_insects_study_years_rest_of_life, data = cohort_abr_sig, family = binomial)
...
```{r}
model_18<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever + gastrointestinal, data = cohort_abr_sig, family = binomial)
...
```{r, error=FALSE}
model_19<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever + gastrointestinal + age_at_neuter_years_quartiles, data = cohort_abr_sig, family = binomial)
...
```{r}
model_21<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles, data = cohort_abr_sig, family = binomial)
...
```{r}
#ever
model_23a<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles , data = cohort_abr_sig, family = binomial)
summary(model_23a)
#5y
model_23b<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_5y + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles , data = cohort_abr_sig, family = binomial)
summary(model_23b)
```
...
```{r}
model_25<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever  + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex, data = cohort_abr_sig, family = binomial)
summary(model_25)
logistic.display(model_25)
model_25a<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever +  gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + heats_before_neutering_terciles, data = cohort_abr_sig, family = binomial)
...
```{r}
model_27<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + neuter, data = cohort_abr_sig, family = binomial)
summary(model_27)
...
```{r}
model_43<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever  + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + neuter + smoke_exposure_study_years_early_life+ all_parasitism, data = cohort_abr_sig, family = binomial)
summary(model_43)
...
```{r}
model_45<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever +   gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + neuter +smoke_exposure_study_years_early_life+ majority_sleep_location_lifetime, data = cohort_abr_sig, family = binomial)
summary(model_45)
...
model_47b<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + neuter +smoke_exposure_study_years_early_life, data = cohort_abr_sig, family = binomial)
summary(model_47b)
...
```{r}
#melanoma
model_48a<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + neuter + smoke_exposure_study_years_early_life + melanoma_present, data = cohort_abr_sig, family = binomial)
summary(model_48a)
...
#lymphoma
model_48b<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever +  gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + neuter + smoke_exposure_study_years_early_life + lymphoma_present, data = cohort_abr_sig, family = binomial)
summary(model_48b)
logistic.display(model_48b)
car::vif(model_48b)
lrtest(model_42,model_48b) 
#MCT
model_48c<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + neuter + smoke_exposure_study_years_early_life + mast_cell_tumor_present, data = cohort_abr_sig, family = binomial)
summary(model_48c)
...
model_48d<- glm(had_HSA ~ age_at_final_date_GB  + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever  + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + neuter + smoke_exposure_study_years_early_life + mast_cell_tumor_present + melanoma_present, data = cohort_abr_sig, family = binomial)
summary(model_48d)
logistic.display(model_48d)
car::vif(model_48d)

#assess if models better with the cancer count or the presence of MCT or melanoma
model_cancer_count <- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever  + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + neuter + smoke_exposure_study_years_early_life + all_cancers_count_no_HSA, data = cohort_abr_sig, family = binomial)
summary(model_cancer_count)

model_melanoma <- glm(had_HSA ~ age_at_final_date_GB  + prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever +   gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + neuter + smoke_exposure_study_years_early_life  +melanoma_present, data = cohort_abr_sig, family = binomial)
...
```{r}
model_f<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA+ prior5y_activity_mode + any_treated_fertilizer_study_years_whole_life + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever + gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + neuter + smoke_exposure_study_years_early_life , data = cohort_abr_sig, family = binomial)
summary(model_f)
...
```{r}
model_f_quart<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_count_no_HSA+ prior5y_activity_mode + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever +  gastrointestinal + avg_weight_5y_prior_endpoint_quartiles + sex + smoke_exposure_study_years_early_life
...
model_f_kg<- glm(had_HSA ~ age_at_final_date_GB  + all_cancers_count_no_HSA+ prior5y_activity_mode + quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever +  gastrointestinal + avg_weight_5y_prior_endpoint_kg_biological + sex + smoke_exposure_study_years_early_life
              , data = cohort_abr_sig, family = binomial)
summary(model_f_kg)
...
NSAID \* smoke only sig - although doesnt seem very meaningful but retain anyways
...
```{r}
model_f<- glm(had_HSA ~ age_at_final_date_GB  + prior5y_activity_mode + all_cancers_YN_no_HSA +quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever +  gastrointestinal + avg_weight_5y_prior_endpoint_kg_biological + sex + smoke_exposure_study_years_early_life+ NSAID_ever*smoke_exposure_study_years_early_life    , data = cohort_abr_sig, family = binomial)
summary(model_f)
logistic.display(model_f)
```

Double check that parasites and freq of activity truly improve model... - AIC smaller without parasite included and LRT not sig
...
Although this does now make NSAID\*smoking interaction and smoking non sig = remove
...
#model without smoking and NSAID quantiles instead
...
doc <- doc %>%
  body_add_flextable(results_flextable) %>%
  body_add_par("Finalfit Regression Results", style = "heading 1")
print(results_flextable, target = here::here("Output/250617_finalfit_logreg_table_all_variables.docx"))
...
lrtest(model_4a,model_5e)
logistic.display(model_5e)
model_5f<- glm(had_HSA ~ age_at_final_date_GB  +all_cancers_YN_no_HSA + prior5y_activity_mode  , data = cohort_abr_sig_5y, family = binomial)
summary(model_5f)
lrtest(model_4a,model_5f)
logistic.display(model_5f)
...
```{r}
model_weed<- glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  +any_treated_weeds_exposed_5yrs_prior, data=cohort_abr_sig_5y,family=binomial)
...
```{r}
model_fert<- glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode +any_treated_weeds_exposed_5yrs_prior + any_treated_fertilizer_exposed_5yrs_prior, data=cohort_abr_sig_5y,family=binomial)
...
```{r}
model_a<- glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode +any_treated_weeds_exposed_5yrs_prior + NSAID_5y, data=cohort_abr_sig_5y,family=binomial)
...
model_a1<- glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode + NSAID_5y, data=cohort_abr_sig_5y,family=binomial)
```
...
```{r}
model_b1 <- glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y + half_anti_inflam_5y_prior_endpoint, data=cohort_abr_sig_5y,family=binomial)
...
model_b2 <- glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y +  quartile_anti_inflam_5y_prior_endpoint, data=cohort_abr_sig_5y,family=binomial)
...
```{r}
model_c1 <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + half_immunologicals_5y_prior_endpoint, data=cohort_abr_sig_5y,family=binomial)
...
model_c2 <- glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y + quartile_immunologicals_5y_prior_endpoint, data=cohort_abr_sig_5y,family=binomial)
...
```{r}
model_d <- glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y + half_anti_parasites_5y_prior_endpoint, data=cohort_abr_sig_5y,family=binomial)
...
```{r}
#sex
model_e1 <- glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex, data=cohort_abr_sig_5y,family=binomial)
...
#sex neut
model_e2 <- glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter, data=cohort_abr_sig_5y,family=binomial)
...
model_e3 <- glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + neuter, data=cohort_abr_sig_5y,family=binomial)
...
model_f <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + region_mode_5y_prior, data=cohort_abr_sig_5y,family=binomial)
summary(model_f)
logistic.display(model_f)
car::vif(model_f)
...
model_g <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + age_at_neuter_years_quartiles, data=cohort_abr_sig_5y,family=binomial)
summary(model_g)
...
```{r}
model_h <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + half_dietary_supplements_5y_prior_endpoint, data=cohort_abr_sig_5y,family=binomial)
summary(model_h)
...
```{r}
model_i <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + fenbendazole_5y, data=cohort_abr_sig_5y,family=binomial)
summary(model_i)
...
model_i2 <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode   + sex_neuter + fenbendazole_5y, data=cohort_abr_sig_5y,family=binomial)
summary(model_i2)
...
```{r}
model_j <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + main_lifestyle_category_mode_5yrs_prior, data=cohort_abr_sig_5y,family=binomial)
summary(model_j)
...
```{r}
model_k <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + half_anti_microbials_5y_prior_endpoint, data=cohort_abr_sig_5y,family=binomial)
summary(model_k)
...
```{r}
model_l <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + avg_purina_BCS_5y_prior_endpoint_quartiles, data=cohort_abr_sig_5y,family=binomial)
summary(model_l)
...
```{r}
model_m <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + lifelong_comorb_present.x, data=cohort_abr_sig_5y,family=binomial)
summary(model_m)
...
```{r}
model_n <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + chronic_inflammatory, data=cohort_abr_sig_5y,family=binomial)
summary(model_n)
...
```{r}
model_o <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + inflammatory_other, data=cohort_abr_sig_5y,family=binomial)
summary(model_o)
...
```{r}
model_p <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + all_parasitism, data=cohort_abr_sig_5y,family=binomial)
summary(model_p)
...
```{r}
model_q <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + steroid_5y, data=cohort_abr_sig_5y,family=binomial)
summary(model_q)
...
```{r}
model_r <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + number_comorbs_present_5y, data=cohort_abr_sig_5y,family=binomial)
summary(model_r)
...
```{r}
model_s <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + use_hepa_filter_exposed_5yrs_prior, data=cohort_abr_sig_5y,family=binomial)
summary(model_s)
...
```{r}
model_t <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + use_hepa_filter_exposed_5yrs_prior, data=cohort_abr_sig_5y,family=binomial)
summary(model_t)
...
```{r}
model_u <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + mode_purina_BCS_5y_prior_endpoint, data=cohort_abr_sig_5y,family=binomial)
summary(model_u)
...
```{r}
model_v<- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + avg_height_5y_prior_endpoint_quartiles, data=cohort_abr_sig_5y,family=binomial)
summary(model_v)
...
```{r}
model_w <- glm(had_HSA ~age_at_final_date_GB + all_cancers_YN_no_HSA + prior5y_activity_mode  + NSAID_5y  + sex_neuter + heating_fuel_primary_mode_5yrs_prior, data=cohort_abr_sig_5y,family=binomial)
summary(model_w)
...
```{r}
model_weight_q <-glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA + avg_weight_5y_prior_endpoint_quartiles+ prior5y_activity_mode  + NSAID_5y  + sex_neuter , data=cohort_abr_sig_5y,family=binomial)
...
model_weight_kg_freq <-glm(had_HSA ~ age_at_final_date_GB + all_cancers_YN_no_HSA +  median_weight_5y_prior_endpoint_kg_outliers_removed_biological + NSAID_5y_quantiles  + sex_neuter +prior5y_activity_mode ,data=cohort_abr_sig_5y,family=binomial)
...
doc <- doc %>%
  body_add_flextable(results_flextable) %>%
  body_add_par("Finalfit Regression Results", style = "heading 1")
print(results_flextable, target = here::here("Output/finalfit_logreg_table_5y_restricted_variables.docx"))
...
Data manip for plotting manually, tidying up variable/category names and making sure baseline variable categories make sense :
...
df <- df %>%
  mutate(
    prior5y_activity_mode = recode(as.character(avg_frequency_5y_prior_endpoint_splits_grouped),
...
final_df[28, 1] <- "Avg. activity freq. score– 5y prior to endpoint (grouped)"
...
ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers2.pdf"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers2.svg"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers2.tif"), plot = forest_log, width = 25, height = 15, dpi = 300)


ggsave(here::here("Output/logreg_forestplot2.pdf"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot2.svg"), plot = forest_log, width = 25, height = 15, dpi = 300)


ggsave(here::here("Output/logreg_forestplot2.tif"), plot = forest_log, width = 25, height = 15, dpi = 300)
```
...
final_df[29, 1] <- "Avg. activity freq. score– 5y prior to endpoint (grouped)"
...
ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers_endo.pdf"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers_endo.svg"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers_endo.tif"), plot = forest_log, width = 25, height = 15, dpi = 300)


ggsave(here::here("Output/logreg_forestplot_2_endo.pdf"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot_2_endo.svg"), plot = forest_log, width = 25, height = 15, dpi = 300)


ggsave(here::here("Output/logreg_forestplot_2_endo.tif"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot_2_endo_2.tif"), plot = forest_log, width = 15, height = 10, dpi = 300)
=======
...
final_df[20, 1] <- "Avg. activity freq. score– 5y prior to endpoint (grouped)"
...
ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers5y.pdf"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers5y.svg"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers5y.tif"), plot = forest_log, width = 25, height = 15, dpi = 300)


ggsave(here::here("Output/logreg_forestplot5y.pdf"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot5y.svg"), plot = forest_log, width = 25, height = 15, dpi = 300)


ggsave(here::here("Output/logreg_forestplot5y.tif"), plot = forest_log, width = 25, height = 15, dpi = 300)
```

Final plot of updated 17/06/25 model where activity modified to just be mode of level
...
#set these as numeric rather than char so that ggplot can use the values for plotting
final_df$conf.high <- as.numeric(final_df$conf.high)
final_df$conf.low <- as.numeric(final_df$conf.low)
final_df$estimate <- as.numeric(final_df$estimate)  
...
```{r}
#makes the OR forest plot bit of the figure
p_mid <- 
  final_df |>
  ggplot(aes(y = fct_rev(AutoNumber))) +
  theme_classic() +
...
# Left plot with more spaced columns and adjusted xlim
#makes the plot of the OR case-noncase table text
p_left <- 
  final_df |>
  ggplot(aes(y = fct_rev(AutoNumber))) + 
  geom_text(aes(x = 0, label = variable), hjust = 0, size = 4, fontface = "bold") +
...
ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers_v2.pdf"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers_v2.svg"), plot = forest_log, width = 25, height = 15, dpi = 300)

ggsave(here::here("Output/logreg_forestplot_casecontrol_numbers_v2.tif"), plot = forest_log, width = 25, height = 15, dpi = 300)


ggsave(here::here("Output/logreg_forestplot_v2.pdf"), plot = forest_log, width = 20, height = 10, dpi = 300)

ggsave(here::here("Output/logreg_forestplot_v2.svg"), plot = forest_log, width = 20, height = 10, dpi = 300)


ggsave(here::here("Output/logreg_forestplot_v2.tif"), plot = forest_log, width = 20, height = 10, dpi = 300)
```
