...
### upon examination of the 5y and ever drug variables dont see how 0 and not recorded distinctions are made = group together. so 1 = had and 0 = no record of drug administered
# Indexes of the columns you want to modify

cols_to_modify <- 253:258  # change as needed

for (i in cols_to_modify) {
  # Temporarily coerce to character to allow replacement
  col_as_char <- as.character(cohort_abr[[i]])
  
  # Replace
  col_as_char[col_as_char == "drug_not_administered"] <- "0"
  
  # Optionally convert back to factor
  cohort_abr[[i]] <- factor(col_as_char)
}
...
###setting "drug_not_administered" as bottom factor level for drug variables as think it actually means drug use not recorded so doesnt make a good baseline
...
for (i in cols_to_modify) {
  ...
  if ("drug_not_administered" %in% current_levels) {
    new_levels <- c(setdiff(current_levels, "drug_not_administered"), "drug_not_administered")
...
cols_to_modify <- 143:185  # adjust as needed
...
for (i in cols_to_modify) {
  ...
  if ("drug_not_administered" %in% current_levels) {
    ...
    # Replace it with NA
    cohort_no_not_rec[[i]][cohort_no_not_rec[[i]] == "drug_not_administered"] <- NA
  }
}
...
```{r}
dependent <- "had_HSA"
explanatory <- c("age_at_final_date_GB","all_cancers_YN_no_HSA","avg_frequency_5y_prior_endpoint_quartiles","quartile_anti_parasites_5y_prior_endpoint","NSAID_ever","gastrointestinal","avg_weight_5y_prior_endpoint_quartiles","sex_neuter")
...
"avg_frequency_5y_prior_endpoint_splits",
```
...
```{r}
dependent <- "had_HSA"
explanatory <- c("age_at_final_date_GB","sex_neuter","all_cancers_YN_no_HSA","avg_frequency_5y_prior_endpoint_quartiles","NSAID_5y")
