...
```{r}
model_f<- glm(had_HSA ~ age_at_final_date_GB  + avg_frequency_5y_prior_endpoint_splits_grouped + all_cancers_count_no_HSA +quartile_anti_parasites_5y_prior_endpoint  +NSAID_ever +  gastrointestinal + avg_weight_5y_prior_endpoint_kg_biological + sex + smoke_exposure_study_years_early_life  , data = cohort_abr_sig, family = binomial)
summary(model_f)
logistic.display(model_f)
```
...
plot2 <-cohort_abr_sig %>%
  or_plot(dependent, explanatory,column_space = c(-0.5, -0.1, 0.5),table_text_size = 3, plot_opts = list(xlim(0, 19)))
plot2
print(plot2)
```
...
Final model output with time restricted variables: H&L p=0.07 C statistics 0.78
...
final_df[13, 2] <- "Weight not recorded"
final_df[12,2]<-">35kg"
final_df[7,2] <- "Female"
final_df[8,2]<- "Male"
final_df[1, 1] <- "Age at final date (Barry)"
final_df[7, 1] <- "Sex"
final_df[14,1] <- "Other cancer/s recorded count"
final_df[9, 1] <- "Avg. adult weight 5y prior to endpoint (quartiles)"
final_df[17, 1] <- "Gastrointestinal comorbidity 5y prior to endpoint"
final_df[19, 1] <- "Anti-parasites - 5y prior to endpoint (quartiles)"
final_df[24, 1] <- "NSAID used - whole life"
final_df[27, 1] <- "Avg. activity freq. score– 5y prior to endpoint (grouped)"
final_df[31, 1] <- "Smoke exposure- whole life"
#replace the inf values from the not recorded category in activty freq with numeric values
final_df[10,5] <-"0.00 (0-3886369.66, p=0.967)"
final_df[10,6] <-"3886369.66"
final_df[10,7]<-"0.967"
...
final_df$category <- factor(final_df$category, 
                        levels = c(">7-<9y","<5y",">5-<7y",">9-<11y",">11-<13y",">13y","Female","Male",">15-<=25kg","<15kg",">25-<=35kg",">35kg","Weight not recorded","No other cancer reported","1 other cancer reported", "2 other cancers reported","GI comorb not present", "GI comorb present","1-2 times",">2-4 times",">4-6 times",">6-20 times","No medications records for this time period", "NSAID not administered", "NSAID administered", "No medications records for lifetime","Daily or more",
                                   "Weekly","Rarely","Activity freq. not recorded","Not within early life", "Within early life"))
...
p_mid <- 
  ...
  labs(x = "Odds Ratio") +
  coord_cartesian(ylim = c(1, 32), xlim = c(0, 12)) +
