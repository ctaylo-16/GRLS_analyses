...
HSA_comorbs2<- HSA_comorbs2 %>%
  group_by(subject_id) %>%
   mutate(
    ...
    years_between_diagnosis_and_last_chronic_inflam = ifelse(is.na(years_between_diagnosis_and_last_chronic_inflam ), "no comorb present", 
...
HSA_comorbs2 <- HSA_comorbs2 %>%
  ...
  mutate(
    years_between_diagnosis_and_last_chronic_inflam = year_in_study_diagnosis_or_final_record_year - study_year_latest_chronic_inflam,
    # Ensure consistency in data type by converting all outputs to character
    years_between_diagnosis_and_last_chronic_inflam = ifelse(
      is.na(years_between_diagnosis_and_last_chronic_inflam), 
      "no comorb present",
      ifelse(years_between_diagnosis_and_last_chronic_inflam < 0, 
             "0", 
             as.character(years_between_diagnosis_and_last_chronic_inflam))
    )
  ) %>%
  ungroup()
...
```{r}
write.csv(HSA_comorbs2,"C:/Users/ctaylor18/GitHub/GRLS_analyses/Output/HSA_cohort_comorbidities.csv")
```
