...
```{r}
write.csv(endpoints_and_all_dogs, here::here("Output/HSA_study_pop_with_final_date.csv"))

write.csv(endpoints_and_all_dogs_study_record_year3, here::here("Output/HSA_study_pop_with_final_date_with_final_study_record_year.csv"))
```
...
meds<- read.csv(here::here("Output/HSA_cohort_medications_5y_and_lifetime.csv"))
...
activity_level_overview<- activity_level_overview %>%
  mutate(across(c(4:6,10:13,15:18,26:29), ~ case_when(
    . == "0" ~ "none",
    . == "0.5" ~ "none to little",
    . == "1" ~ "little",
...
#rename had_HSA.x to had_HSA and final_date.y to final_date
merged_HSA_data2 <- merged_HSA_data2%>%
  dplyr::rename(
    final_date = final_date.x
  )

#drop all cols that end in .y or .x
merged_HSA_data2 <- merged_HSA_data2 %>%
 dplyr::select(
    -ends_with(".x"),
    -ends_with(".y")
...
#write to csv
write.csv(merged_HSA_data2,here::here("Output/HSA_cohort_all_RFs_data.csv"))
```
