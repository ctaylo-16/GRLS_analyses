...
MDI_dogs <- MDI_dogs %>%
  dplyr::select(c(subject_id,MDI_quintile,MDI.rate,owner_MDI.rate,owner_MDI_quintile))
#enviro
...
# Modify your pipeline to include mode calculation for purina_BCS
...
merged_HSA_data <- merged_HSA_data %>%
  mutate(across(c(387:446), ~ replace(., is.na(.), "medication_use_not_recorded")))
