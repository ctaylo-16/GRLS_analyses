...
#comorb
comorb<-read.csv("C:/Users/ctaylor18/GitHub/GRLS_analyses/Output/HSA_cohort_comorbidities.csv")
comorb <- comorb %>%
  dplyr::select(-c("X.1","X","chronic_inflam_present.y"))
comorb <- comorb %>%
  rename(chronic_inflam_present = chronic_inflam_present.x)
#activity
...
merged_HSA_data <- merged_HSA_data %>%
  mutate(across(c(189:249), ~ replace(., is.na(.), "medication_use_not_recorded")))
