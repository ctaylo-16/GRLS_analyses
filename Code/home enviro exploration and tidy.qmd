...
```{r}
write.csv(result_combined,here::here("Output/house_location_all_rows.csv"))
```
...
end_points_all <- end_points %>%
  ...
  distinct(subject_id,.keep_all=TRUE)
write.csv(end_points_all,here::here("Output/endpoints_all_house_location.csv"))

#end points lymphoma
end_points_lymphoma_join <- end_lymphoma %>%
  left_join(result_combined,by="subject_id") %>%
  distinct(subject_id,.keep_all=TRUE)
write.csv(end_points_lymphoma_join,here::here("Output/endpoints_lymphoma_house_location.csv"))
#end points HSA
end_points_HSA_join <- end_HSA %>%
  left_join(result_combined,by="subject_id") %>%
  distinct(subject_id,.keep_all=TRUE)
write.csv(end_points_HSA_join,here::here("Output/endpoints_HSA_house_location.csv"))
...
write.csv(HSA_cohort_location_house_per_dog,here::here("Output/HSA_cohort_house_location_enviro_exposures.csv"))
```
