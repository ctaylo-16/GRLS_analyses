...
total_dose <- function(data,dose,duration,mapped_values_frequency){
  ...
  mutate(across(c(dose, duration, mapped_values_frequency), as.numeric)) %>%
    #calculate row wise so each record has a dose calculated
  rowwise() %>%
    #prod is product of these 3 columns multiplied and NA ignored
  mutate(total_dosage = prod(c(dose, duration, mapped_values_frequency), na.rm = TRUE))
