...
cancer_count_summary <- HSA_comorbs_unique5 %>%
  group_by(subject_id) %>%
  summarise(
    ...
    #keep max value for each dog ie. 1 if present 
    malig_cancers_count = max(malig_cancers_count, na.rm = TRUE),
    benign_cancers_count = max(benign_cancers_count, na.rm = TRUE),
    all_cancers_count = max(all_cancers_count, na.rm = TRUE),
    malig_cancers_YN = max(malig_cancers_YN, na.rm = TRUE),
    benign_cancers_YN = max(benign_cancers_YN, na.rm = TRUE),
    all_cancers_YN = max(all_cancers_YN, na.rm = TRUE),
    .groups = "drop"
  )
#replace -Inf with 0
cancer_count_summary[cancer_count_summary == -Inf] <- 0
#now remove the cancer cols from the df so that they dont mess up at next step joining / distinct
HSA_comorbs_unique5_no_cancer <- HSA_comorbs_unique5 %>%
  dplyr::select(-c(malig_cancers_count,malig_cancers_YN,benign_cancers_count,benign_cancers_YN,all_cancers_count,all_cancers_YN))
```
...
df_result7_distinct <- df_result7 %>%
  distinct(subject_id,.keep_all = TRUE)
HSA_comorbs_unique4_df7_distinct <- HSA_comorbs_unique4_distinct_cancer %>%
...
HSA_comorbs_unique4_df7_distinct_abr <- HSA_comorbs_unique4_df7_distinct %>%
  dplyr::select(c(1,273:303))

write.csv(HSA_comorbs_unique4_df7_distinct,"C:/Users/ctaylor18/GitHub/GRLS_analyses/Output/HSA_cohort_comorbs_incl_cancer.csv")
```
