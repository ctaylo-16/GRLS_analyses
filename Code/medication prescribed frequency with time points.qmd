...
## Exploratory - Medications length of time/number of times prescribed:
...
write.csv(HSA_5y_and_lifetime_all_dogs,here::here("Output/HSA_cohort_medications_5y_and_lifetime.csv"))
```

### Medications to signal comorbidities & cross check with comorbidities dogs

```{r}
lifetime_comorbs <- read.csv(here::here("Output/HSA_cohort_comorbidities.csv"))
```
...
write.csv(all_unique_med_ingredients_or_names,here::here("Output/GRLS_all_unique_medication_names_or_ingredients.csv"))
```
...
First for ectoparasite tx:

```{r}
#use ecto list above and medication ingredients column to find dogs where ectoparasiticeds used in 5y prior to endpoint
HSA_medication_5y_filt <- HSA_medication_5y_filt %>%
...
#manually using quantile values generated from above
...
```{r}
#code to identify for each row if endoparasite tx was recorded
HSA_medication_5y_filt <- HSA_medication_5y_filt %>%
...
write.csv(ecto_endo_NSAID,here::here("Output/HSA_cohort_medications_5y_and_lifetime.csv"))
```

### Get all medications data for endpoint dogs for 5y prior to endpoint for GRLS clinical records cross-checking:

```{r}
write.csv(endpoints_medication_5y_filt,here::here("Output/GRLS_endpoints_5y_prior_medications.csv"))
```
