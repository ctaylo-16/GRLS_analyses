...
medication_summary <- meds_per_dog %>%
  ...
  summarise(
    median_count_per_dog = median(count, na.rm = TRUE),
    mean_count_per_dog = mean(count, na.rm = TRUE),
    min_count_per_dog = min(count, na.rm = TRUE),
    max_count_per_dog = max(count, na.rm = TRUE),
    Q1 = quantile(count, 0.25, na.rm = TRUE),
    Q3 = quantile(count, 0.75, na.rm = TRUE)
  )
```

Now use these quartiles to add back onto :
```{r}
# Step 3: Join quartile cutoffs back to individual dog data
meds_per_dog_quart <- meds_per_dog %>%
  ...
  select(subject_id, active_ingredient_groups, quartile)

# Step 4: Reshape so each medication group is a column with Q1-Q4 values
meds_wide_quart <- meds_per_dog_quart %>%
  pivot_wider(names_from = active_ingredient_groups, values_from = quartile)
...
lapply(meds_wide_no_sub, function(x) table(x, useNA = "ifany"))
...
```{r}
meds_per_dog_half <- meds_per_dog %>%
  left_join(medication_summary, by = "active_ingredient_groups") %>%
  mutate(
    halves = case_when(
      count <= median_count_per_dog ~ "lower_half",
      count > median_count_per_dog ~ "upper_half"
    )
  ) %>%
  select(subject_id, active_ingredient_groups, halves)


# Step 4: Reshape so each medication group is a column with Q1-Q4 values
meds_wide_half <- meds_per_dog_half %>%
  pivot_wider(names_from = active_ingredient_groups, values_from = halves)

```

```{r}
colSums(is.na(meds_wide_half))

meds_wide_no_sub_half <- meds_wide_half %>%
  dplyr::select(-subject_id)
lapply(meds_wide_no_sub_half, function(x) table(x, useNA = "ifany"))
```
...
# Step 2: Get summary stats across all dogs for each medication group
lifetime_medication_summary <- lifetime_meds_per_dog %>%
  group_by(active_ingredient_groups) %>%
  summarise(
    median_count_per_dog = median(count, na.rm = TRUE),
    mean_count_per_dog = mean(count, na.rm = TRUE),
...
meds_per_dog_quart_lifetime <- lifetime_meds_per_dog %>%
  left_join(lifetime_medication_summary, by = "active_ingredient_groups") %>%
  mutate(
    quartile = case_when(
      count <= Q1 ~ "Q1",
      count > Q1 & count <=median_count_per_dog ~ "Q2",
...
```{r}
HSA_5y_and_lifetime <- meds_wide_quart_lifetime %>%
