...
#remove the earlier 5y columns
GRLS_summary_lifelong_2 <- GRLS_summary_lifelong %>%
  dplyr::select(c(subject_id,25:30))
```
...
```{r}
write.csv(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid,"C:/Users/ctaylor18/GitHub/GRLS_analyses/Output/GRLS_lymphoma_variables_dataset_interim.csv")
```
...
profile_abr <- profile %>%
  dplyr::select(subject_id,neuter, sex, birth_date2, age_at_neuter_years,spay_neuter_date2)
...
#check female variables not include males
table(neut_data_single_row$age_first_heat_years)
#males should not have a variable value (it will be 0)
M <- neut_data_single_row %>%
  filter(sex=="M")
#if male then replace all Os with NA
neut_data_single_row <- neut_data_single_row %>%
  mutate(
    across(
      c(age_first_heat_years, heat_last_year_num_exposures, preg_pre_neuter),  # <- your female-specific columns
      ~ if_else(sex == "M", NA, .)
    )
  )
...
neut_data_single_row <- neut_data_single_row %>%
  mutate(
    across(
      c(age_first_heat_years, heat_last_year_num_exposures, preg_pre_neuter,mated_pre_neuter),  # <- neut-specific columns
      ~ if_else(neuter == "Entire", NA, .)
    )
  )
#keep relevant cols
neut_data_single_row <- neut_data_single_row %>%
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_neutering2 <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_neutering2 %>%
  dplyr::select(-c(neuter.x,sex.x,birth_date2.x,record_date2.x,age_at_neuter_years.x,spay_neuter_date2.x))
...
dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_neutering3_weight4_abr <- dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_neutering3_weight4 %>%
  dplyr::select(subject_id,277:287)
...
char_cols <- sapply(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_neutering3_weight4_activity_endpoint_activity2, is.character)
sapply(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_neutering3_weight4_activity_endpoint_activity2[, char_cols], function(x) length(unique(x)))


lapply(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_neutering3_weight4_activity_endpoint_activity2[, char_cols], unique)
...
```{r}
table(dog_pop_deprivation_enviro_smoking_sleep_lifestyle_all_house_location_comorbs_meds_NSAID_steroid_neutering3_weight4_activity_endpoint_activity2$inflammatory_other_5y2)
```
