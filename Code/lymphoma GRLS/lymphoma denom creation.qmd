...
> **Workflow status:** This notebook is retained as the future cohort-refresh route. The current reproducible workflow starts from `Data/lymphoma_cohort_241201.csv`, which is censored at 1 December 2024. Refresh that cohort from updated manually coded clinical records before the next substantive lymphoma analysis. See `docs/pipelines.md`.
...
dog_profiles <- read.csv(here::here("Data/dog_profile.csv"))
...
```{r}
med_q <- read.csv(here::here("Data/medications.csv"))
...
```{r}
write.csv(dates_and_lymphoma_diagnosis,here::here("Data/lymphoma_cohort_241201.csv"))
```
