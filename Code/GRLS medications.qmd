...
dash_pattern <- grepl("^\\d+(\\.\\d+)?-\\d+(\\.\\d+)?$", dose$dose)
...
# Regex to match units with optional whitespace
unit_pattern <- "\\b(mg|ml|tbs|tbsp|tsp|g|mcg|gm|tube|drop|drops|spray|sprays)\\b\\s*"

# Identify rows where dose doesn't have a comma and isn't a pure number
no_comma_non_numeric <- grepl("^[^,]*$", dose$dose) & !grepl("^\\d+(\\.\\d+)?$", dose$dose)

# Strip the specific units and capture them into `dose_unit`
dose$dose_unit[no_comma_non_numeric] <- regmatches(dose$dose[no_comma_non_numeric], regexpr(unit_pattern, dose$dose[no_comma_non_numeric], ignore.case = TRUE))

# Remove the unit from the `dose` column (if it exists)
dose$dose[no_comma_non_numeric] <- gsub(unit_pattern, "", dose$dose[no_comma_non_numeric], ignore.case = TRUE)

# Trim any extra whitespace in `dose`
dose$dose <- trimws(dose$dose)
...
non_numeric_dose5 <- non_numeric_dose5 %>%
