...
medications <- read.csv(here::here("Data/medications.csv"),na.strings=c ("","NA"))
vaccinations <-read.csv(here::here("Data/vaccines.csv"),na.strings=c ("","NA"))
OTC_meds <- read.csv(here::here("Data/over_the_counter_medications.csv"),na.strings=c ("","NA"))
parasite_control <-read.csv(here::here("Data/flea_tick_heartworm.csv"),na.strings=c ("","NA"))
...
end_points <- read.csv(here::here("Data/study_endpoints.csv"))
...
write.csv(med_ingredients_count,here::here("Output/medications_all.csv"))
```
...
# Function to add quotes around each element in a list
quote_drugs <- function(drug_list) {
  # Use `paste()` to add quotes around each drug and combine them into a single string
  return(paste(sprintf('"%s"', drug_list), collapse = ", "))
}

# Create a data frame with two columns: "Drug_Group" and "Drug"
combined_df <- data.frame(
  Drug_Group = c("Anti-Histamine", "Anti-Inflammatory", "Anti-Microbials", "Anti-Parasites", 
                 "Anti-Septics", "Anti-Viral", "Cardio-Respiratory", "Dietary Supplements",
                 "Diuretics", "Enteric", "Fluid Metabolites", "Hormones and Related", 
                 "Immunologicals", "Misc","Neurological"),
  Drugs = c(
    quote_drugs(anti_histamine),   # Apply function to each list
    quote_drugs(anti_inflam),
    quote_drugs(anti_microbials),
    quote_drugs(anti_parasites),
    quote_drugs(anti_septics),
    quote_drugs(anti_viral),
    quote_drugs(cardio_respiratory),
    quote_drugs(dietary_supplements),
    quote_drugs(diuretics),
    quote_drugs(enteric),
    quote_drugs(fluid_metabolites),
    quote_drugs(hormones_and_related),
    quote_drugs(immunologicals),
    quote_drugs(misc),
    quote_drugs(neurological)
  ),
...
write.csv(combined_df, here::here("Output/med_groups_table.csv"), row.names = FALSE, quote = TRUE)
...
-   specific parasiticide ones eg. 460Â mgÂ lufenuron,Â 23Â mgÂ milbemycinÂ oxime?
...
```{r}
dog_pe <- read.csv(here::here("Data/exam_physical.csv"))
...
write.csv(medications_5,here::here("Output/GRLS_medications_initial_tidy.csv") )
...
#write this to csv 
write.csv(medications_mapped,here::here("Output/GRLS_medications_initial_tidy.csv"))
...
```{r}
vaccinations <-read.csv(here::here("Data/vaccines.csv"))
...
```{r}
flea_tick_heartworm <-read.csv(here::here("Data/flea_tick_heartworm.csv"))
```
